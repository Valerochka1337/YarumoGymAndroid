...
class BackendSyncTest : RoomDaoTest() {
  ...
  @Test
  fun `negotiated strength records round trip alongside their workout without changing profile wire`() = runTest {
    ...
    val strengthId = UUID.nameUUIDFromBytes("ValerochkaGym.strength-planner-profile.v1:user-a".encodeToByteArray()).toString()
    val effortId = UUID.nameUUIDFromBytes("ValerochkaGym.workout-effort.v1:user-a:$workoutId".encodeToByteArray()).toString()
    insertWorkout(workoutId, finishedAt = 2000)
    raw.execSQL("INSERT INTO strength_planner_profiles(scope,syncId,updatedAt) VALUES('user-a',?,1)", arrayOf(strengthId))
    raw.execSQL("INSERT INTO workout_efforts(workoutId,scope,syncId,updatedAt,effort) VALUES(?,'user-a',?,1,'HARD')", arrayOf(workoutId, effortId))
    ...
    assertEquals("HARD", server.records.getValue("workout_effort:$effortId").payload?.get("effort")?.jsonPrimitive?.content)
  ...
  fun `share import preserves strength capability local effort and exact queued bytes`() = runTest {
    ...
    raw.execSQL("INSERT INTO workout_efforts(workoutId,scope,syncId,updatedAt,effort) VALUES(?,'user-a',?,1,'HARD')", arrayOf(workoutId, effortId))
    sync.run()
    raw.execSQL("UPDATE workout_efforts SET effort='EASY',updatedAt=2 WHERE workoutId=?", arrayOf(workoutId))
    val payload = PortableData(raw).snapshot().getValue("workout_effort:$effortId")
    val change = CloudChange("workout_effort", effortId, server.records.getValue("workout_effort:$effortId").revision, false, payload)
    val exact = "{ \"operationId\": \"retained-strength-effort\", \"changes\": [ ${Json.encodeToString(change)} ], \"catalogRevision\": null }"
    raw.execSQL("INSERT INTO backend_outbox(id,owner,requestJson) VALUES(1,'user-a',?)", arrayOf(exact))
    val imported = server.addSharedRoutine()
    val posts = server.postAttempts
    sync.applyImportedRoutine(requireNotNull(store.snapshot()), imported, server.revision)
    assertTrue(server.lastRawHeaders.getValue("X-Gym-Capabilities").split(',').contains("strength-planner-personalization"))
    assertNotNull(db.routineDao().getRoutineBySyncId(imported))
    assertEquals("EASY", PortableData(raw).snapshot().getValue("workout_effort:$effortId")["effort"]?.jsonPrimitive?.content)
    assertEquals(posts, server.postAttempts)
    raw.query("SELECT requestJson FROM backend_outbox WHERE id=1").use {
      assertTrue(it.moveToFirst())
