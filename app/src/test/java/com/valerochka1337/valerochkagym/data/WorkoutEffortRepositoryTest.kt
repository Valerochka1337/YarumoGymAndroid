...
class WorkoutEffortRepositoryTest : RoomDaoTest() {
  ...
  @Test
  fun `only completed workouts accept effort and clearing retains an explicit record`() = runTest {
    val store = Store()
    ...
    assertEquals(
        ...
        repo.save(requireNotNull(repo.captureTarget("active")), WorkoutEffort.HARD),
    )
  ...
  @Test
  fun `same owner login invalidates an already open effort editor`() = runTest {
    val store = Store()
  ...
  @Test
  fun `session changes during a write roll back effort`() = runTest {
    val store = Store()
    val sync = BackendSync(db, Server, store)
    sync.claim("owner")
    val tokens = BackendTokens("owner", "owner@example.com", "access", "refresh")
